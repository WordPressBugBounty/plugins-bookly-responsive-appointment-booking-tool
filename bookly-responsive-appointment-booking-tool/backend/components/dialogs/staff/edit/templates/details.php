...
<form class="bookly-js-staff-details">
    <div class="row">
        ...
        <div class="col">
            ...
            <div class="form-group">
                <label for="bookly-full-name"><?php esc_html_e( 'Full name', 'bookly-responsive-appointment-booking-tool' ) ?></label>
    ...
    <?php if ( Common::isCurrentUserAdmin() ) : ?>
        <div class="form-group">
            <label for="bookly-wp-user"><?php esc_html_e( 'User', 'bookly-responsive-appointment-booking-tool' ) ?></label>
            <select class="form-control custom-select" name="wp_user_id" id="bookly-wp-user">
                <option value=""><?php esc_html_e( 'Select from WordPress users', 'bookly-responsive-appointment-booking-tool' ) ?></option>
            ...
            <small class="form-text text-muted">
                <?php esc_html_e( 'If this staff member requires separate login to access personal calendar, a regular WP user needs to be created for this purpose.', 'bookly-responsive-appointment-booking-tool' ) ?>
                <?php esc_html_e( 'User with "Administrator" role will have access to calendars and settings of all staff members, user with another role will have access only to personal calendar and settings.', 'bookly-responsive-appointment-booking-tool' ) ?>
                <?php esc_html_e( 'If you leave this field blank, this staff member will not be able to access personal calendar using WP backend.', 'bookly-responsive-appointment-booking-tool' ) ?>
            </small>
        </div>
    <?php endif ?>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                <label for="bookly-email"><?php esc_html_e( 'Email', 'bookly-responsive-appointment-booking-tool' ) ?></label>
        ...
        <div class="col-sm-12">
            <div class="form-group">
                <label for="bookly-phone"><?php esc_html_e( 'Phone', 'bookly-responsive-appointment-booking-tool' ) ?></label>
    ...
    <div class="form-group">
        <label for="bookly-info"><?php esc_html_e( 'Info', 'bookly-responsive-appointment-booking-tool' ) ?></label>
        <textarea id="bookly-info" name="info" rows="3" class="form-control"><?php echo esc_textarea( $staff->getInfo() ) ?></textarea>
        <small class="form-text text-muted"><?php printf( esc_html__( 'This text can be inserted into notifications with %s code', 'bookly-responsive-appointment-booking-tool' ), '{staff_info}' ) ?></small>
    </div>
    ...
    <div class="form-group" id="bookly-visibility" data-default="<?php echo esc_attr( $staff->getVisibility() ) ?>">
        <label><?php esc_html_e( 'Visibility', 'bookly-responsive-appointment-booking-tool' ) ?></label>
        <div class="custom-control custom-radio">
            ...
            <label for="bookly-visibility-public" class="custom-control-label"><?php esc_html_e( 'Public', 'bookly-responsive-appointment-booking-tool' ) ?></label>
        </div>
        <div class="custom-control custom-radio">
            ...
            <label for="bookly-visibility-private" class="custom-control-label"><?php esc_html_e( 'Private', 'bookly-responsive-appointment-booking-tool' ) ?></label>
        </div>
        <?php if ( Config::proActive() || $staff->getVisibility() === 'archive' ) : ?>
            <div class="custom-control custom-radio">
                ...
                <label for="bookly-visibility-archive" class="custom-control-label"><?php esc_html_e( 'Archive', 'bookly-responsive-appointment-booking-tool' ) ?></label>
            </div>
        <?php endif ?>
        <small class="form-text text-muted"><?php esc_html_e( 'To make staff member invisible to your customers set the visibility to "Private".', 'bookly-responsive-appointment-booking-tool' ) ?></small>
    </div>
    ...
    <div class="bookly-js-modal-footer">
        <?php if ( Common::isCurrentUserAdmin() ) : ?>
            ...
            <?php Buttons::render( null, 'btn-danger ladda-button bookly-js-staff-archive', __( 'Archive', 'bookly-responsive-appointment-booking-tool' ), ! Config::proActive() || $staff->getVisibility() == 'archive' ? array( 'style' => 'display:none;' ) : array(), '<i class="fas fa-fw fa-archive mr-1"></i>{caption}' ) ?>
        <?php endif ?>
        <?php Buttons::render( 'bookly-details-save', 'btn-success bookly-js-save', __( 'Save', 'bookly-responsive-appointment-booking-tool' ) ) ?>
    </div>
</form>